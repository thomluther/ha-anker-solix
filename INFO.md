...
> [!IMPORTANT]
> Avoid automation duplicates of status requests via the interval entity and your own automation. If the status request interval is being enabled, regular button press actions are no longer required abd should be removed from any custom automation.

You can verify active status request intervals via the HA developper states panel, by filtering all anker solix interval entities.
